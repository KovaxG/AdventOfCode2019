...
runUntilOutputOrHaltWith :: Int -> Computer -> Computer
runUntilOutputOrHaltWith input comp =
  go $ comp {inputs=inputs comp ++ [input]}
  where
    hasOutput :: Computer -> Bool
    hasOutput = not . null . outputs

    go :: Computer -> Computer
    go c
      | isTerminal c  || hasOutput c = c
      | otherwise = go $ tick c

runLoopAmps :: Int -> [Computer] -> Int
runLoopAmps input comps@(c:cs)
  | all isTerminal comps = input
  | otherwise =
    let c2 = runUntilOutputOrHaltWith input c
        output = outputOf c2
        c3 = c2 { outputs = [] }
    in runLoopAmps output (cs ++ [c3])
    where
      outputOf :: Computer -> Int
      outputOf = head . outputs
